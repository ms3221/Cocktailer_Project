    ...
    methods = {
          ...
          function onMSGestureChange(e) {
            ...
            if (!scrolling || Number(new Date()) - startT > 500) {
              ...
              if (!fade && slider.transitions) {
                if (!slider.vars.animationLoop) {
                  dx =
                    accDx / ((slider.currentSlide === 0 && accDx < 0) || (slider.currentSlide === slider.last && accDx > 0) ? Math.abs(accDx) / cwidth + 2 : 1);
                }
                slider.setProps(offset + dx, "setTouch");
              }
            }
      ...
      smoothHeight: function (dur) {
        if (!vertical || fade) {
          ...
          dur
            ? $obj.animate({ height: slider.slides.eq(slider.animatingTo).innerHeight() }, dur).css("overflow", "visible")
      ...
      if (!fade) {
        ...
        if (type === "init") {
          if (!touch) {
            ...
            slider.slides
              .css({ opacity: 0, display: "none", webkitTransition: "opacity " + slider.vars.animationSpeed / 1000 + "s ease", zIndex: 1 })
              .eq(slider.currentSlide)
